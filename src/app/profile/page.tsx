...
export default function ProfilePage() {
  ...
  const initialName = authUser?.first_name?.trim() || user.displayName;
  const initialDescription = authUser?.description ?? user.bio ?? "";
  const [profileName, setProfileName] = useState(initialName);
  const [profileDescription, setProfileDescription] = useState(initialDescription);
  const [profileSaving, setProfileSaving] = useState(false);
  const displayName = authUser?.first_name?.trim() || user.displayName;
  const avatarUrl = authUser?.avatar_url || user.avatar;
  const balls = authUser?.balls ?? user.rating;
  const kpdPercent = Math.round((authUser?.kpd ?? 0) * 100);
  const nextLevelBalls = authUser?.next_level_balls ?? 1000;
  const registeredAt = authUser?.created_at ?? user.createdAt;
  ...
  const mergedFromMe = {
    site: { ...defaultSettings.site, ...((authUser?.settings as { site?: Partial<ProfileSettings["site"]> } | undefined)?.site ?? {}) },
    notifications: { ...defaultSettings.notifications, ...((authUser?.settings as { notifications?: Partial<ProfileSettings["notifications"]> } | undefined)?.notifications ?? {}) },
    general: { ...defaultSettings.general, ...((authUser?.settings as { general?: Partial<ProfileSettings["general"]> } | undefined)?.general ?? {}) },
  };
  ...
  const fetchMyQuestions = useCallback(
    async (
      ...
      if (append) setUserQuestionsLoadingMore(true);
      else setUserQuestionsLoading(true);
      try {
  ...
  useEffect(() => {
    ...
    api
      .get<{ my_questions?: ProfileQuestionItem[] }>("v1/me")
      .then((data) => {
        const first = data.my_questions ?? [];
        setUserQuestions(first);
        setUserQuestionsPage(1);
        setUserQuestionsLastPage(first.length >= 5 ? 2 : 1);
      })
      .catch(() => {
        setUserQuestions([]);
      });
