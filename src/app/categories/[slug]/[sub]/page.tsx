...
export default function SubcategoryPage() {
  ...
  // Название подкатегории с заглавной буквы
  ...
  const getFilteredQuestions = () => {
    ...
    switch (activeTab) {
      case 0:
        return filtered.filter((q) => q.status === "opened");
      case 1:
        return filtered.filter((q) => q.status === "voting");
      case 2:
      case 3:
        return filtered.filter((q) => q.status === "closed");
      case 4:
        return filtered.filter((q) => (q as any).isPremium === true);
      default:
        return filtered;
    }
  };

  const filteredCategoryQuestions = getFilteredQuestions();
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="breadcrumbs">
          ...
          <span className="breadcrumbs__sep">•</span>
          <Link href={`/categories/${slug}`} className="breadcrumbs__link">
            {category.name}
          </Link>
          <span className="breadcrumbs__sep">•</span>
          <span className="breadcrumbs__current">{subName}</span>
        </div>

        {/* Блок вопросов подкатегории */}
