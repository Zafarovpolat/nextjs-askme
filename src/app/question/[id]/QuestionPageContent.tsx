...
// Склонение для блока «Похожие вопросы»
...
export default function QuestionPageContent({ initialQuestion }: { initialQuestion: QuestionPageData }) {
  ...
  const renderAnswer = (answer: MockAnswer, isBest = false, isNested = false, mainTitle?: string) => (
    <div
            ...
            {sidebarCategories.map(cat => (
              ...
              >
                <div className="quest_catogory_title" onClick={() => toggleCategory(cat.slug)}>
                  ...
                  <svg width="12" height="12" className="quest_catogory_arrow">
                    <use xlinkHref="#arrow-bottom"></use>
                  </svg>
                </div>
                <div className="quest_catogory_list">
                  {cat.subcategories.map(sub => (
                    <Link href="#" key={sub}>{sub}</Link>
                  ))}
                </div>
              </div>
            ))}
        ...
        <div className="question_main_list">
            ...
            <div className="question_list_item-info">
              <div className="question_list_item_left">
                <Link href={`/profile/${initialQuestion.author.id}`}>
                  <img src={initialQuestion.author.avatar_url || '/images/icons/avatar.svg'} alt="" />
                </Link>
                <div>
                  <Link href={`/profile/${initialQuestion.author.id}`} className="main_text">
                    {initialQuestion.author.full_name}
                  </Link>
                  <div className="quest_user_title">
                    <span>{initialQuestion.author.level_name ?? 'Участник'}</span>
                  </div>
          ...
          {bestAnswer && (
            <div className="best-comments" style={{ display: 'block' }}>
              ...
              {renderAnswer(bestAnswer, true, false, initialQuestion.title)}
            </div>
          )}
